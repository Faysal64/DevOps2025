...
install -d -o stunnel4 -g stunnel4 -m 0755 "$STUNNEL_PID_DIR"
install -d -o stunnel4 -g stunnel4 -m 0750 "$STUNNEL_LOG_DIR"

# Activer stunnel côté Debian/Ubuntu
echo 'ENABLED=1' | tee /etc/default/stunnel4 >/dev/null

# Libérer le port local s'il est déjà pris
fuser -k "${LOCAL_LISTEN_PORT}"/tcp 2>/dev/null || true
...
systemctl unmask stunnel4 2>/dev/null || true
update-rc.d stunnel4 defaults >/dev/null 2>&1 || true
service stunnel4 restart || service stunnel4 start
...
for i in {1..20}; do
  ...
  if [ "$i" -eq 20 ]; then
    ...
    [ -f "${STUNNEL_LOG_DIR}/client.log" ] && tail -n 80 "${STUNNEL_LOG_DIR}/client.log" || true
    exit 1
  fi
done
...
find "${SCRIPT_DIR}" -maxdepth 1 -type f -name "$(basename "${OVPN}").bak.*" -print -delete 2>/dev/null || true
# Créer un nouveau backup propre
cp -a "${OVPN}" "${OVPN}.bak.$(date +%Y%m%d%H%M%S)"   # Backup
sed -i -E \
...
touch "$OVPN_LOG" && chmod 640 "$OVPN_LOG"
openvpn --config "${OVPN}" --log "$OVPN_LOG"
...
_vpn_start_proc() { openvpn --config "$VPN_CONF" ${OVPN_LOG:+--log "$OVPN_LOG"}; }
_vpn_stop_proc()  { pgrep -f "openvpn.*--config[= ]${VPN_CONF}" >/dev/null 2>&1 \
                      && pkill -f "openvpn.*--config[= ]${VPN_CONF}" \
                      || pkill -x openvpn >/dev/null 2>&1 || true; }

vpn-start()   { _vpn_require_conf || return 1;
                [ -n "${STUNNEL_SERVICE:-}" ] && { service "$STUNNEL_SERVICE" restart >/dev/null 2>&1 || service "$STUNNEL_SERVICE" start >/dev/null 2>&1 || true; }
...
vpn-restart() { _vpn_require_conf || return 1; _vpn_stop_proc;
                [ -n "${STUNNEL_SERVICE:-}" ] && { service "$STUNNEL_SERVICE" restart >/dev/null 2>&1 || true; }
...
stunnel-start()  { [ -n "${STUNNEL_SERVICE:-}" ] && service "$STUNNEL_SERVICE" start  && echo "✅ stunnel démarré"  || echo "stunnel non configuré"; }
stunnel-stop()   { [ -n "${STUNNEL_SERVICE:-}" ] && service "$STUNNEL_SERVICE" stop   && echo "🛑 stunnel arrêté"   || echo "stunnel non configuré"; }
